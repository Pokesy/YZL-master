...
public class RegisterActivity extends BaseBActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    juese = arr.size() > 1 ? arr.get(1).get("id") : "";
    opinion = getIntent().getExtras().getString("opinion");
    mVerifyStep.entry();
    if (arr.size() > 0 && TextUtils.equals(arr.get(0).get("open_invite"), "0")) {
        ...
        new RsenUrlUtil.OnJsonResultListener<DiscoverFragment.FXBean>() {
          ...
          public void onResult(boolean state, List beans) {
            if (state) {
              ...
              ToastHelper.showToast("用户已存在或被禁用", RegisterActivity.this);
    ...
    RsenUrlUtil.execute(this, RsenUrlUtil.URL_VERIFY,
        new RsenUrlUtil.OnJsonResultListener<VBean>() {
          ...
          public void onResult(boolean state, List<VBean> beans) {
            ...
            if (state) {
              //ToastHelper.showToast(beans.get(0).message, RegisterActivity.this);
              mRegisterStep.entry();
            } else {
              ToastHelper.showToast("验证码错误", RegisterActivity.this);
            }
          }
        });
  }
    ...
    public void onClick(View v) {
      ...
      switch (mId) {
        ...
        case R.id.openRegist:
          Intent intent1 = new Intent(RegisterActivity.this, BeforeRegistetActivity.class);
    ...
    public void handleMessage(Message msg) {
      ...
      try {
        ...
        if (jsonObject.getString("success").equals("true")) {
          mNext.setEnabled(true);
          Toast.makeText(RegisterActivity.this, jsonObject.getString("message"), Toast.LENGTH_SHORT)
              .show();
        } else {
          mNext.setEnabled(false);
          Toast.makeText(RegisterActivity.this, jsonObject.getString("message"), Toast.LENGTH_SHORT)
    ...
    public void handleMessage(android.os.Message msg) {
      ...
      try {
        ...
        if (jsonObject.getBoolean("success")) {
          ...
          progressDialog.cancel();
          ToastHelper.showToast(jsonObject.getString("message"), RegisterActivity.this);
        }
      } catch (JSONException e) {
        e.printStackTrace();
      }
  ...
  public class VerifyStep implements Step {
    ...
    public void next() {
      if (!TextUtils.isEmpty(verifyId.getText().toString()) && !TextUtils.isEmpty(mName.getText()
        ...
        Toast.makeText(RegisterActivity.this, "手机号或验证码不能为空", Toast.LENGTH_SHORT).show();
      }
    }
    ...
    public void next() {
      ...
      if (opinion.equals("0")) {
        if (!username.matches("[0-9A-Za-z_]*")) {
          Toast.makeText(RegisterActivity.this, "用户名只能是英文数字和下划线", Toast.LENGTH_SHORT).show();
        } else if (username.length() > 8 || username.length() < 0) {
          Toast.makeText(RegisterActivity.this, "用户名不能小于0或者大于8字符", Toast.LENGTH_SHORT).show();
          return;
        }
      }
      if (opinion.equals("1")) {
        if (!username.matches(
          ...
          Toast.makeText(RegisterActivity.this, "邮箱不正确", Toast.LENGTH_SHORT).show();
          return;
        }
      }
      if (opinion.equals("2")) {
        if (!username.matches("^(13|15|17|18)\\d{9}$")) {
          Toast.makeText(RegisterActivity.this, "号码不正确", Toast.LENGTH_SHORT).show();
          return;
        }
      }
      ...
      if (!username.equalsIgnoreCase(null) && !password.equalsIgnoreCase(null) && !"".equals(
        ...
        Toast.makeText(RegisterActivity.this, "请先填写昵称或密码", Toast.LENGTH_LONG).show();
      }
    }
