  ...
  private void initView() {
    ...
    Comment_Edit.setOnFocusChangeListener(new View.OnFocusChangeListener() {
      ...
      public void onFocusChange(View v, boolean hasFocus) {
        RelativeLayout.LayoutParams layoutParams = new RelativeLayout.LayoutParams(RelativeLayout
            .LayoutParams.FILL_PARENT, RelativeLayout.LayoutParams.WRAP_CONTENT);
        if (hasFocus) {
    ...
    Comment_Send.setOnClickListener(new View.OnClickListener() {
      public void onClick(View v) {
        if (!Url.SESSIONID.equals("")) {
          if (Comment_Edit.getText().toString().trim().length() <= 5 || Comment_Edit.getText()
            ...
            if (commentType == 2) {
              weiApi.sendWeiboComment(weiboInfo.getWid(), Comment_Edit.getText().toString(),
                  Integer.parseInt(weiboCommentInfo.getId()));
              Log.e("BUG1111>>>>>>", weiboInfo.getWid() + Comment_Edit.getText().toString() +
                  weiboCommentInfo.getId());
            } else {
    ...
    mFaceBtn.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View v) {
        ...
        imm.hideSoftInputFromWindow(Comment_Edit.getWindowToken(), InputMethodManager
            .HIDE_NOT_ALWAYS);
    ...
    mFaceBtn.setOnTouchListener(new TouchHelper(SendCommentActivity.this, "#BBBBBB", "#F9F9F9",
        "color"));


    final int maxCommentCount = getResources().getInteger(R.integer.comment_max_length);
    mCountView.setText(maxCommentCount + "/" + maxCommentCount);
    ...
    Comment_Edit.addTextChangedListener(new TextWatcher() {
      ...
      public void onTextChanged(CharSequence s, int start, int before, int count) {
        mCountView.setText(maxCommentCount - s.length() + "/" + maxCommentCount);
      }
