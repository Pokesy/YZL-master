...
//            HttpGet httpGet=new HttpGet("http://demo.opensns.cn/api
// .php?s=/issue/sendIssue&session_id="+session_id+"&issue_id="+21+"&cover_id="+222+"&title
// ="+title+"&content="+content+"&url="+url);
        String uriAPI = Url.HTTPURL + "?s=" + Url.ADDEVENTS + "&session_id=" + session_id +
            "&cover_id=" + IMG_ID + "&title=" + URLEncoder.encode(title, "UTF-8") + "&address=" +
            URLEncoder.encode(place, "UTF-8") + "&sTime=" + SDATETIME + "&eTime=" + EDATETIME +
            "&deadline=" + SPDATETIME + "&explain=" + URLEncoder.encode(content, "UTF-8") +
            "&type_id=" + mukuai + "&limitCount=" + num;
        Log.d("网址:", uriAPI);
...
//                progressDialog.show();
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    adapter = new ArrayAdapter<String>(PostEventActivity.this, R.layout
        .simple_list_item_activity_1);
