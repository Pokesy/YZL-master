...
public class GroupFloorReplyActivity extends BaseBActivity implements View.OnClickListener {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    bodyScroView.setOnTouchListener(new View.OnTouchListener() {
      @Override public boolean onTouch(View v, MotionEvent event) {
        ...
        imm.hideSoftInputFromWindow(replyEdtext.getWindowToken(),
  ...
  private void InitFloorView(HashMap<String, String> floorInfoMap) {
    ...
    ImageLoader.loadOptimizedHttpImage(mContext, floorInfoMap.get("user_logo")).into(floorUserHead);
  ...
  @Override public void onClick(View v) {
    ...
    switch (viewID) {
      ...
      case R.id.send_reply_btn:
        if (replyEdtext.getText().length() > 0 && !"".equals(
            replyEdtext.getText().toString().trim())) {
          sendReplyBtn.setBackgroundResource(R.drawable.border);
          sendReplyBtn.setTextColor(Color.parseColor("#A9ADB0"));
          replyBox.setVisibility(View.GONE);
          //自动隐藏软键盘
          InputMethodManager imm2 =
              (InputMethodManager) getSystemService(Context.INPUT_METHOD_SERVICE);
          if (imm2.isActive()) {
            imm2.hideSoftInputFromWindow(replyEdtext.getWindowToken(),
                InputMethodManager.HIDE_NOT_ALWAYS);
          }
          ...
          groupApi.replyComment(String.valueOf(post_id), ReplyId + "",
              replyEdtext.getText().toString());
