...
//            JSONObject comObject=jsonObject.getJSONObject("comment");
      ...
      UserInfoModel userInfo = getUserInfo(comObject.getJSONObject("user"));
...
//            JSONObject weiboObject=detailObject.getJSONObject("weibo");
      ...
      UserInfoModel user = getUserInfo(weiboObject.getJSONObject("user"));
...
//            JSONObject jsonObject1= (JSONObject) jsonObject.get("message");
    ...
    try {
      ...
      for (int i = 0; i < jsonArray.length(); i++) {
        ...
        try {
          ...
          UserInfoModel user = getUserInfo(listitem.getJSONObject("user"));
          weiboInfo.setUser(user);
  ...
  public WeiboInfo getRepostWeiboInfo(String value) {
    ...
    try {
      ...
      if (jsonSourse.getString("user").equals("null")) {
        ...
        UserInfoModel userInfo = getUserInfo(jsonSourse.getJSONObject("user"));
        repostWeibo.setUser(userInfo);
    ...
    try {
      ...
      for (int i = 0; i < jsonArray.length(); i++) {
        ...
        if (jsonObj.getString("user").equals("null")) {
          ...
          UserInfoModel userInfo = getUserInfo(jsonObj.getJSONObject("user"));
          weiboCommentInfo.setUser(userInfo);
          list.add(weiboCommentInfo);
        }
      }
  ...
  public UserInfoModel getUserInfo(JSONObject user) {
    UserInfoModel userInfo = new UserInfoModel();
    try {
      userInfo.setUid(user.getString("uid"));
      userInfo.setAvatar(Url.IMAGE + user.getString("avatar128"));
  ...
  public UserInfoModel getUserInfo(com.alibaba.fastjson.JSONObject user) {
    UserInfoModel userInfo = new UserInfoModel();
  ...
  public UserInfoModel getUserAllInfo(String result) {
    UserInfoModel userInfo = new UserInfoModel();
    try {
      JSONObject jsonObject = new JSONObject(result);
...
//            userInfo.setCtime(getStandardDate(jsonObject.getString("reg_time")));
      ...
      userInfo.setIs_follow(jsonObject.optInt("is_follow"));
...
//            Log.d("myjson>>>",jsonObject.getString("birthday"));
      ...
      userInfo.setBirthday(jsonObject1.optString("生日"));
  ...
  // 解析附近用户的方法
  public List<UserInfoModel> getNearUserList(String result) {
    List<UserInfoModel> list = null;
    try {
      JSONArray jay = new JSONArray(result);
      list = new ArrayList<UserInfoModel>();
      for (int i = 0; i < jay.length(); i++) {
        JSONObject job = jay.getJSONObject(i);
  ...
  public PostInfo getPostInfo(String result) {
    ...
    com.alibaba.fastjson.JSONObject jsonObject = (com.alibaba.fastjson.JSONObject) JSON.parse
        (result);
...
//    public DiscoverInfo getDiscoverInfo(String result){
//        DiscoverInfo postInfo= new DiscoverInfo();
//
//        com.alibaba.fastjson.JSONObject jsonObject= (com.alibaba.fastjson.JSONObject) JSON
// .parse(result);
//        postInfo.setImgList(getImgUrlList(jsonObject.getJSONObject("content").getJSONArray
// ("image")));
//        postInfo.setPostContent(jsonObject.getJSONObject("content").getString("content"));
//        postInfo.setReplyCount(jsonObject.getString("create_time"));
//        postInfo.setPostTitle(jsonObject.getString("title"));
  ...
  public List<PostInfo> getPostInfos(String result, String forumTitle) {
    ...
    try {
      ...
      for (int i = 0; i < jsonArray.length(); i++) {
        JSONObject listItem = (JSONObject) jsonArray.opt(i);
        PostInfo postInfo = new PostInfo();
        String s = listItem.getString("create_time");
//               postInfo.setCreatTime(getStandardDate(s));
        postInfo.setCreatTime(s);
...
//               postInfo.setLastReplyTime(getStandardDate(listItem.getString("last_reply_time")));
        postInfo.setLastReplyTime(listItem.getString("last_reply_time"));
  ...
  public List<ForumInfo> getForumInfos(String result) {
    ...
    try {
      ...
      for (int i = 0; i < jsonArray.size(); i++) {
        com.alibaba.fastjson.JSONObject listItem = (com.alibaba.fastjson.JSONObject) jsonArray
            .get(i);
            ...
            try {
              ...
              if (tempJsonArray != null) {
                for (int j = 0; j < tempJsonArray.length(); j++) {
                  ...
                  secondNewsCategory.setCan_post(tempJsonArray.optJSONObject(j).getString
                      ("can_post"));
                  secondNewsCategory.setNeed_audit(tempJsonArray.optJSONObject(j).getString
                      ("need_audit"));
  ...
  public ArrayList<NewsListInfo> getNewsList(String result) {
    ...
    try {
      ...
      for (int i = 0; i < jsonArray.length(); i++) {
        ...
        newsListInfo.setCover(baseApi.getResourcesURL(jsonArray.optJSONObject(i).getString
            ("cover")));
...
//                    imageInfo.setSrc(baseApi.getResourcesURL(imageJSONArray.optJSONObject(i)
// .getString("src")));
