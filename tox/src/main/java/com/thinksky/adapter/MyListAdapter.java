      ...
      if (repostWeiboType.equalsIgnoreCase("image")) {
        ...
        if (imglist.size() >= 3) {
          ...
          for (int i = 0; i < 3; i++) {
            ...
            loadWeiboImg(hold, hold.imgViewList.get(i), Url.USERHEADURL + "/" + imglist.get(i),
                finalBitmap);
          }
        } else {
...
//		hold.Up.setTag(list.get(arg0).getQid());
  ...
  public void loadWeiboImg(Holder hold, final ImageView weiboImg, final String url, FinalBitmap
      finalBitmap) {
    weiboImg.setTag(url);
    ImageLoader.loadOptimizedHttpImage(ctx, url).placeholder(R.drawable.picture_no).into(weiboImg);
  }
