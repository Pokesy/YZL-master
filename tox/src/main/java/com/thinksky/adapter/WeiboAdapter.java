...
public class WeiboAdapter extends BaseAdapter {
  ...
  public View getView(final int arg0, View arg1, ViewGroup arg2) {
    ...
    if (arg1 == null) {
      ...
      hold.imgViewList.get(1).setVisibility(View.GONE);
      hold.imgViewList.get(2).setVisibility(View.GONE);
      hold.imgViewList.get(0).setImageResource(R.drawable.picture_no);
      hold.imgViewList.get(1).setImageResource(R.drawable.picture_no);
...
// .replace("cn///","cn/").replace("cn//","cn/").replace("net///","net/").replace("net//","net/");
  ...
  private void setWeiboImgVisiable(Holder holder, int num) {
    ...
    if (num < holder.imgViewList.size()) {
      for (int i = holder.imgViewList.size() - 1; i < num; i--) {
        holder.imgViewList.get(i).setImageResource(R.drawable.picture_no);
        holder.imgViewList.get(i).setVisibility(View.GONE);
      }
    }
  }

  public void loadWeiboImg(Holder hold, final ImageView weiboImg, String url, FinalBitmap
      finalBitmap, KJBitmap kjBitmap) {
    String url2;
    if (url.startsWith("http")) {
      url2 = url;
    } else {
...
// .replace("cn///","cn/").replace("cn//","cn/").replace("net///","net/").replace("net//","net/");
  ...
  private void startPhotoBrowser(int arg0, int index) {
    ...
    if (weiboInfo.getType().equalsIgnoreCase("repost")) {
      Intent intent = new Intent(ctx, ImagePagerActivity.class);
      Bundle bundle = new Bundle();
      bundle.putStringArrayList("image_urls", (ArrayList<String>) weiboInfo.getRepostWeiboInfo()
          .getImgList());
      bundle.putInt("image_index", index);
      intent.putExtras(bundle);
      ctx.startActivity(intent);
