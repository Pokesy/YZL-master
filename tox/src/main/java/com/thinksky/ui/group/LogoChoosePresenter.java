...
import com.thinksky.utils.DeviceUtils;
 ...
 */
public class LogoChoosePresenter {
  ...
  public LogoChoosePresenter(Activity context) {
    this.mContext = context;
    inject();
  }

  private void inject() {
    DaggerServiceComponent.builder().globalModule(new GlobalModule((BaseApplication) mContext
  ...
  public void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    switch (requestCode) {
      case LogoChoosePresenter.REQUEST_CODE_CAPTURE:
        break;
      case LogoChoosePresenter.REQUEST_CODE_ALBUM:
  ...
  public void uploadData() {
    ...
    String url = "api.php?s=public/uploadCover";
    Subscription subscription = mUserService.upload(url, body).subscribeOn(Schedulers.io())
        .observeOn
            (AndroidSchedulers.mainThread())
        .subscribe(new UiRpcSubscriberSimple<UploadModel>(mContext) {
          @Override
          protected void onSuccess(UploadModel model) {
            ((BaseApplication) mContext.getApplication()).getGlobalComponent().getGlobalBus()
                .post(new UploadLogoEvent(model.getId(), model.getUrl()));
          }
  ...
  public class UploadLogoEvent {
    public String id;
    public String path;

    public UploadLogoEvent(String id, String path) {
      this.id = id;
      this.path = path;
    }
  }
