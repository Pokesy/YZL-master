...
//                    bean.memberCount = jsonObject.getString("memberCount");
          ...
          bean.user_logo = RsenUrlUtil.URL_BASE + jsonObject.getJSONObject("user").getString
              ("avatar32");
    ...
    protected void onBindView(RViewHolder holder, int position, final RemenhuatiBean bean) {
      ...
      if (bean.imgList != null && bean.imgList.size() > 0) {
        ...
        for (int i = 0; i < size; i++) {
          ...
          if (imageView != null) {
            com.thinksky.utils.imageloader.ImageLoader.loadOptimizedHttpImage(getActivity(), url)
                .into(imageView);
