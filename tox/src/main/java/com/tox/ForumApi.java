...
public class ForumApi extends BaseApi {
     ...
     * @param content
     * @param attachIds
     * &session_id=&signature=&email=&nickname=&Sex=& isfactory=
     * & factory_name=& longitude=& latitude=
     * & isdisplay=& mobile=& data=&address=
     */
    public void sendDiscoverPost(String title, String content, List<String> attachIds, String forumId) {
        putArg("longitude", forumId);
        putArg("factory_name", title);
        putArg("mobile", content);
        if (attachIds.size() != 0) {
            putArg("data", WeiboApi.getAttachIds(attachIds));
        }
        execute(handler, Url.getApiUrl(Url.SENDDISCOVER), true);
    }

    public void sendQuestion(String title, String content, String category_id, List<String> attachIds,String session_id, String score) {
        putArg("session_id", session_id);
        putArg("title", title);
        putArg("content", content);
        putArg("category_id", category_id);
        putArg("score", score);
        if (attachIds.size() != 0) {
            putArg("attach_id", WeiboApi.getAttachIds(attachIds));
        }
        execute(handler, Url.getApiUrl(Url.SENDQUESTION), true);
    }
}
