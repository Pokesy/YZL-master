...
public class GroupApi extends BaseApi {
  ...
  public HashMap<String, String> getGroupInfoMap(String result, HashMap<String, String> map) {
    try {
      ...
      map.put("group_logo", Url.IMAGE + jsonObjFist.getString("logo"));
      ...
      map.put("create_time", jsonObjFist.getString("create_time"));
      map.put("notice", jsonObjFist.optString("notice"));
      ...
      map.put("user_avatar128", Url.IMAGE + tempObj.getString("avatar128"));
      return map;
  ...
  //全部群组信息
  public ArrayList<JSONObject> getAllGroupList(String url, int page, int type_id, String
      session_id) {
    ...
    try {
      ...
      HttpGet get = new HttpGet(String.valueOf(Url.HTTPURL + url) + "&page=" + page + "&type_id="
          + type_id + "&width=300&height=300" + "&session_id=" + session_id);
  ...
  public ArrayList<JSONObject> getGroupType(String url, int id) {
    ...
    try {
      jsonList = new ArrayList<JSONObject>();
      while (true) {
        HttpGet get = new HttpGet(String.valueOf(Url.HTTPURL + url) + "&page=" + count + "&id=" +
            id);
        count = count + 1;
        HttpResponse httpResponse = httpClient.execute(get);
        if (httpResponse.getStatusLine().getStatusCode() == 200) {
  ...
  //群组帖子信息
  public ArrayList<JSONObject> getGroupPostList(String url, int page, int group_id, int cateID,
                                                String session_id) {
    ...
    try {
      jsonList = new ArrayList<JSONObject>();
      HttpGet get = new HttpGet(String.valueOf(Url.HTTPURL + url) + "&page=" + page +
          "&group_id=" + group_id + "&cate_id=" + cateID + "&session_id=" + session_id);
      HttpResponse httpResponse = httpClient.execute(get);
      if (httpResponse.getStatusLine().getStatusCode() == 200) {
        result = EntityUtils.toString(httpResponse.getEntity());
        try {
  ...
  public ArrayList<JSONObject> getPostReply(String url, String post_id, int page) {
    ...
    try {
      jsonList = new ArrayList<JSONObject>();
      HttpGet get = new HttpGet(String.valueOf(Url.HTTPURL + url) + "&post_id=" + post_id +
          "&page=" + page);
      HttpResponse httpResponse = httpClient.execute(get);
      if (httpResponse.getStatusLine().getStatusCode() == 200) {
        result = EntityUtils.toString(httpResponse.getEntity());
  ...
  public ArrayList<JSONObject> getLzlReply(String url, String post_id, int to_f_reply_id, int page) {
    ...
    try {
      jsonList = new ArrayList<JSONObject>();
      HttpGet get = new HttpGet(String.valueOf(Url.HTTPURL + url) + "&post_id=" + post_id +
          "&to_f_reply_id=" + to_f_reply_id + "&page=" + page);
      HttpResponse httpResponse = httpClient.execute(get);
      if (httpResponse.getStatusLine().getStatusCode() == 200) {
        result = EntityUtils.toString(httpResponse.getEntity());
        try {
